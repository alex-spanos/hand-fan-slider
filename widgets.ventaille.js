Widgets.Ventailles = {
	VentailleItem: function(){
		var _p = Relator.$(), browser = Widgets.Helpers.Detect.browser,
			...
			cssAnim = Widgets.Helpers.Detect.features.css.transitions,
			...
			animPosition = cssAnim ?
				function(that, position, duration, callback) {
					...
					duration = 10*Math.ceil(duration/10);
		...
		function load(that) {
			...
			_.Image = $('<img>').load(function(){
				_.Click.append(this);
				_.Click.on('vclick', function(e) {
					e.preventDefault();
					window.location.href = _.Click.attr('href');
				});
	...
	Ventaille: function(){
		var _p = Relator.$(), util = Widgets.Helpers, drag = util.Detect.browser.drag;
		...
		function C(selfId, options) {
			...
			_.Label = $self.find('.'+sel.label).eq(0);
		...
		function label(that) {
			var _ = _p.get(that);
			_.Label.text(_.Items[Math.floor(_.Visible/2)+_.Position].Options.Image.Title);
		}
